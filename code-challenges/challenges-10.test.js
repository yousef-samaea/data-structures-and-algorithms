...
const getNames = (arr) => {
  let arrRev = arr.map((value ) => {
    let arrStr = value.name.split('');
    let revArr = arrStr.reverse();
    let arrJoin = revArr.join('');
    return arrJoin;
  });
  return arrRev;
};
...
const count = (target, input) => {
  ...
  return input.reduce((result, arr) => {
    let rep  = arr.reduce((acc, number) => target === number ? acc += 1 : acc ,0);
    return result + rep ;
  }, 0);
};
...
const totalSum = (input) => {
  let nArr = input.map(arr =>{
    let c = arr.reduce((acc,el) => {
      acc = acc + el;
      return acc;
    },0);
    return c;
  });
  let fSum = nArr.reduce((acc,el) => {
    acc = acc + el;
    return acc;
  },0);
  return fSum;
...
let findShortest = (data) => {
  // Solution code here...
  let shortObj = data.reduce((shortChar, character) => {
    if(parseInt(character.height) < parseInt(shortChar.height)) return character;
    else return shortChar;
  }, data[0]);

  return shortObj.name;
};

...
function transformToLis(obj){
  let fArr = Object.entries(obj);
  let secArr = [];
  for (let i = 0; i < fArr.length; i++) {
    secArr.push(`<li>${fArr[i][0]}: ${fArr[i][1]}</li>`);
  }
  return secArr;
}
...
const addValues = (arr) => {
  let nArr = arr.reduce((red,value,index)=>{
    red = red + value;
    return red;
  },0);
  return nArr;
...
const countNumberOfElements = (arr) => {
  let arrL = arr.reduce((red,value,index)=>{
    index++;
    return index;
  });
  return arrL;
